...
θˆ = argmin (νˆ − ν(θ))T Ω (νˆ − ν(θ)) θ∈Θ
where νˆ is the WV estimated from the sample, ν(θ) is the WV implied by the model Fθ and Ω is an appropriately chosen positive definite weighting matrix. Under certain regularity conditions, this estimator is consistent and asymptotically normal for many time series models (see Guerrier et al., 2013 for details).
...
In the next two sections, we present a simulation study to investigate the finite-sample performance as well as the robustness properties of the (R)GMWM and then an example which illustrates the practical benefits of the GMWM for some engineering applications.
